...
def printLoadBar(ticketCt, ticketTotal):
        print str(int((ticketCt/ticketTotal) * 100)) + "% Complete   \r",
        return
...
ticketCt      = 0.0
ticketTotal   = len(ticketHistory["result"])
for ticket in ticketHistory["result"]:
        ...
        bugState            = int(ticket["state"])
        printLoadBar(ticketCt, ticketTotal)
        if bugState == 7 or bugState == 6:
                bugIds              = bugIds + [bugID]
                ...
                #note that the missing break is intentional
                #it increases the priority of these bugs
                for bugWord in bugSubjTags:
